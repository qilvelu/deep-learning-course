...
#相当于将Q和K拼接（第一次）
        ...
        attention_weight = F.softmax(score_Q_K, dim=1)
        attention_QKV = torch.bmm(attention_weight.unsqueeze(0), en_outputs.unsqueeze(0))
...
torch.save(encoder, 'encoder_attention')
torch.save(decoder, 'decoder_attention')"""
'''
# 训练完毕
exit(0)
'''

encoder = torch.load('encoder_attention',weights_only=False)
decoder = torch.load('decoder_attention',weights_only=False)
