...
test_loader = DataLoader(dataset=test_dataset,  #打包
                          ...
                          shuffle=True)
# ============= 以下构建模型 =============================
# 修复：改用当前版本的torchvision重新构建模型，替代版本不兼容的googlenet_base5
...
#用 googlenet-1378be20 中的参数值更新模型 googlenet base
...
def getAccOnadataset(data_loader):
    ...
    with torch.no_grad():
        for i, (x, y) in enumerate(data_loader):
            ...
            output = googlenet_model(x) # 返回tuple (主输出, aux1, aux2)
            # 修复：取主输出（第一个元素）
